...
class ActiveDANND3Trainer:
	...
	def train_stage(self):
		...
		self.logger.info(f"#######  active DANN D3 training on {dataset_name} {len(self.source_labeled_set)} source({source_domain_name}) and {len(self.target_labeled_set)} target data")
		...
		while iter_cnt < expect_iter:
			...
			src_tgt_concat_discrim_domains = (concat_discrim_domains>0).long()
			student_src_discrim_loss = self.dom_loss_weight * F.cross_entropy(student_concat_discrim_logits[:, :2], src_tgt_concat_discrim_domains)
			
			# target alignment
			student_allway_discrim_logits = torch.cat([student_concat_discrim_logits[:, 0:1], student_concat_discrim_logits[:, 2:]], dim=1)
			student_tgt_discrim_loss = self.dom_loss_weight * self.indom_aux_loss_weight * F.cross_entropy(student_allway_discrim_logits, concat_discrim_domains)
			...
			wandb.log({
				...
				f"train/stage-{self.active_stage}/stu_src-discrim-loss": student_src_discrim_loss.detach().cpu().item(),
				f"train/stage-{self.active_stage}/stu_tgt-discrim-loss": student_tgt_discrim_loss.detach().cpu().item(),
				'train_iter': iter_cnt
			})
