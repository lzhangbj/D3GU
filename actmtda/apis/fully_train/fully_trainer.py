...
class FullyTrainer:
	...
	def run(self):
		...
		optimizer = build_optimizer(self.net, self.cfg)
		...
		self.logger.info(f"####### supervised training on {dataset_name} {len(self.source_dataloader.dataset)} source({source_domain_name}) and {len(self.target_train_dataloader.dataset)} target data")
		...
		while iter_cnt < expect_iter:
			...
			loss = self.loss_fn(labeled_cls_logits, labeled_labels)
			...
			if iter_cnt % self.cfg.TRAIN.ITER_REPORT == 0:
				...
				self.logger.info(
					'ETA:{}, Epoch:{}/{}, iter:{}/{}, lr:{:.5f}, loss:{:.4f}'.format(
						eta,
			...
			if iter_cnt % (self.cfg.TRAIN.VAL_EPOCH * n_iter_per_epoch) == 0:
				...
				for target_domain in range(len(target_domain_names)):
					...
					wandb_log_dict[f"val/target_{target_domain_names[target_domain]}_best_acc"] = max_target_accs[
						target_domain]
