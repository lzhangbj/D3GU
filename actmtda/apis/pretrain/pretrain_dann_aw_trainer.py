...
class PretrainDANNAwTrainer:
	def __init__(self, cfg):
		...
		self.use_domain_prob_weighting = self.cfg.LOSS.DOMAIN_PROB_WEIGHTING
		self.cheat_dom = self.cfg.LOSS.CHEAT_DOM
		self.cheat_dom_weight = self.cfg.LOSS.CHEAT_DOM_WEIGHT
		...
		while iter_cnt < expect_iter:
			...
			if self.use_domain_prob_weighting:
				student_discrim_loss = (self.dom_loss_weight * (self.n_target+1) * student_concat_discrim_probs
				                        * F.cross_entropy(student_concat_discrim_logits, concat_domains, reduction='none')).mean()
			else:
				student_discrim_loss = self.dom_loss_weight * F.cross_entropy(student_concat_discrim_logits, concat_domains, weight=weight)
