...
class PretrainDANND3Trainer:
	def __init__(self, cfg):
		...
		# source val dataset
		if self.cfg.DATASET.DATASET_NAME == 'domain-net':
			source_val_dataset = build_dataset(self.cfg.DATASET, self.cfg.DATASET.SOURCE.DOMAIN_NAMES, domain_label_start=0, split='test', is_train=False)
		else:
			source_val_dataset = build_dataset(self.cfg.DATASET, self.cfg.DATASET.SOURCE.DOMAIN_NAMES, domain_label_start=0, split=None, is_train=False)
	...
	def run(self):
		...
		self.logger.info(f"####### random ensembled DANN d3 pretrain for dataset {dataset_name}-{source_domain_name} #########")
		...
		interval_student_src_discrim_loss = AverageMeter()
		interval_student_tgt_discrim_loss = AverageMeter()
		...
		while iter_cnt < expect_iter:
			...
			# src-target alignment
			src_tgt_concat_domains = (concat_domains>0).long()
			student_src_discrim_loss = self.dom_loss_weight * F.cross_entropy(student_concat_discrim_logits[:, :2], src_tgt_concat_domains)
			
			# target alignment
			student_allway_discrim_logits = torch.cat([student_concat_discrim_logits[:, 0:1], student_concat_discrim_logits[:, 2:]], dim=1)
			student_tgt_discrim_loss = self.dom_loss_weight * self.indom_aux_loss_weight * F.cross_entropy(student_allway_discrim_logits, concat_domains)
			
			student_cls_loss = F.cross_entropy(student_src_cls_logits, source_labels)
			
			# update student model
			student_loss = student_cls_loss + student_src_discrim_loss + student_tgt_discrim_loss
			...
			interval_student_src_discrim_loss.update(student_src_discrim_loss.detach().cpu().item(), B_source)
			interval_student_tgt_discrim_loss.update(student_tgt_discrim_loss.detach().cpu().item(), B_source)
			
			wandb.log({
				f"train/stu_loss": student_loss.detach().cpu().item(),
				f"train/stu_cls-loss": student_cls_loss.detach().cpu().item(),
				f"train/stu_src-discrim-loss": student_src_discrim_loss.detach().cpu().item(),
				f"train/stu_tgt-discrim-loss": student_tgt_discrim_loss.detach().cpu().item(),
				'train_iter': iter_cnt
			})
			...
			if iter_cnt % self.cfg.TRAIN.ITER_REPORT == 0:
				...
				log_info += f"student-loss / cls-loss / src-discrim-loss / tgt-discrim-loss: {interval_student_loss.avg:.4f} / {interval_student_cls_loss.avg:.4f} / {interval_student_src_discrim_loss.avg:.4f} / {interval_student_tgt_discrim_loss.avg:.4f}"
				self.logger.info(log_info)
				
				interval_student_loss.reset()
				interval_student_cls_loss.reset()
				interval_student_src_discrim_loss.reset()
				interval_student_tgt_discrim_loss.reset()
			...
			if iter_cnt % (self.cfg.TRAIN.VAL_EPOCH * n_iter_per_epoch) == 0:
				...
				with torch.no_grad():
					...
					for batch_data in self.source_val_dataloader:
						...
						student_discrim_logits = self.student_discriminator(feats)
						student_source_src_probs = torch.softmax(student_discrim_logits[:, :2], dim=1).detach().cpu().numpy()[:, 0:1]
						student_source_tgt_probs = torch.softmax(student_discrim_logits[:, 2:], dim=1).detach().cpu().numpy()
						student_source_probs = np.concatenate([student_source_src_probs, student_source_tgt_probs], axis=1)
					...
					for tgt_dataset_idx, target_val_dataloader in enumerate(self.target_val_dataloaders):
						for batch_data in target_val_dataloader:
							...
							student_discrim_logits = self.student_discriminator(feats)
							student_target_src_probs = torch.softmax(student_discrim_logits[:, :2], dim=1).detach().cpu().numpy()[:, 0:1]
							student_target_tgt_probs = torch.softmax(student_discrim_logits[:, 2:], dim=1).detach().cpu().numpy()
							student_dom_probs = np.concatenate([student_target_src_probs, student_target_tgt_probs], axis=1)
