...
@HEAD.register("normw-fc")
class NormWeightFCHead(nn.Module):
	def __init__(self, cfg):
		super(NormWeightFCHead, self).__init__()
		...
		self.w_params = nn.Parameter(torch.ones(input_dim, output_dim) / input_dim)
	...
	def forward(self, x):
		for fc in self.fcs:
			x = fc(x)
		normed_params = F.normalize(self.w_params, dim=0, p=2)
		x = x @ normed_params
		return x
