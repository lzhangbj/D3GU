...
@SAMPLER.register("target-combined_entropy_sampler")
class TargetCombinedEntropySampler(BaseSampler):
	def __init__(self, *args, **kwargs):
		super(TargetCombinedEntropySampler, self).__init__(*args, **kwargs)
	...
	def __call__(self, model, discriminator=None, *args, **kwargs):
		...
		with torch.no_grad():
			...
			for batch_data in target_dataloader:
				...
				entropy = - (probs * torch.log(probs+1e-8)).mean(dim=1).detach().cpu().numpy()
